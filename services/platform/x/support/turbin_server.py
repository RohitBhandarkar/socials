...
def _log(message: str, verbose: bool, is_error: bool = False):
    if verbose or is_error:
        ...
        console.print(f"[turbin_server.py] {timestamp}|[{color}]{log_message}[/{color}]")


class TurbinRequestHandler(SimpleHTTPRequestHandler):
...
def start_review_server(profile_name: str, port: int = 8765, verbose: bool = False):
    ...
    handler_factory = lambda *args, **kwargs: TurbinRequestHandler(*args, root_dir=root_dir, verbose=verbose, **kwargs)
    httpd = HTTPServer(('127.0.0.1', port), handler_factory)
    _log(f"Serving Turbin review for '{profile_name}' at http://127.0.0.1:{port}", verbose)
